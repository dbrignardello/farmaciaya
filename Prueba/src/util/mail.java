package util;
	...
	public class mail {
	    ...
	    public static void config(String... namesAndValues) {
	        ...
	        for (int i = 0; i < namesAndValues.length - 1; i += 2) {
	            ...
	            switch    (nameConfig) {
	                case "username":
	                case "login":
	                    mail.login = valueConfig;
	                    break;
	                case "password":
	                case "pass":
	                    mail.password = valueConfig;
	                    break;
	                case "to":
	                    mail.to = valueConfig;
	                    break;
	                case "title":
	                case "subject":
	                    mail.subject = valueConfig;
	                    break;
	                case "msg":
	                case "body":
	                    mail.body = valueConfig;
	            }
	        }
	    }
	    public static Boolean send(String... namesAndValues) {
	        mail.config(namesAndValues);
	        try {
	            return mail.send();
	    ...
	    public static Boolean send() throws AddressException, MessagingException {
	        ...
	        if (mail.cc != null) {
	            generateMailMessage.addRecipient(Message.RecipientType.CC, new InternetAddress(mail.cc));
	        }
	        if (mail.bcc != null) {
	            generateMailMessage.addRecipient(Message.RecipientType.BCC, new InternetAddress(mail.bcc));
	        }
	        if (mail.subject == null) {
	            mail.subject = "subject gmail object";
	        }
	        generateMailMessage.setSubject(mail.subject);
	        if (mail.body == null) {
	            mail.body = "<h1>body gmail object</h1><p>it's a simple test</p>";
	        }
